...
export async function GET(req: NextRequest) {
  ...
  if (employeeId) {
    requests = getRequestsForEmployee(employeeId);
  } else if (statusFilter === 'pending') {
    requests = getPendingRequests();
  } else {
    // All requests (manager view)
    const { getStore } = await import('@/lib/hcm-store');
    requests = Array.from(getStore().requests.values());
  }
...
export async function POST(req: NextRequest) {
  ...
  if (willSilentlyFail()) {
    ...
    // Return success but balance is NOT decremented — silent failure
    // _silentFailure lives inside data so hcmFetch passes it through to onSuccess
